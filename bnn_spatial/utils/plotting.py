...
def plot_spread(domain, samples, n_keep=10, color="xkcd:bluish"):
    """
    ...
    ax.plot(domain, mu, lw=lw, color='xkcd:red')  # mean of all samples
...
def plot_cov_heatmap(covs, titles, domain):
    """
...
def plot_samples_2d(samples, extent, obs=None, figsize=(14,13)):
    ...
    ims = [ax[i, j].imshow(next(samples_it), cmap='Spectral_r', extent=extent, origin='lower', aspect='equal')
           for i, j in product(range(4), range(4))]
