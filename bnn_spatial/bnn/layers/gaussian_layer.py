...
class GaussianLayer(nn.Module):
    def __init__(self, input_dim, output_dim, rbf_dim=None, prior_per='layer', fit_means=True, nonstationary=False):
        """
        ...
        if self.nonstationary:
            ...
            self.W_rho_coeffs = nn.Parameter(torch.randn(self.W_shape), requires_grad=True)
            self.b_rho_coeffs = nn.Parameter(torch.randn(self.b_shape), requires_grad=True)
        else:
            if fit_means:
            ...
            self.W_rho = nn.Parameter(torch.ones(self.W_shape), requires_grad=True)
            self.b_rho = nn.Parameter(torch.ones(self.b_shape), requires_grad=True)
    ...
    def reset_parameters(self):
        ...
        """
        if self.nonstationary:
            init.normal_(self.W_rho_coeffs)
            init.normal_(self.b_rho_coeffs)
            if self.fit_means:
                init.zeros_(self.W_mu_coeffs)
                init.zeros_(self.b_mu_coeffs)
        else:
            init.ones_(self.W_rho)
            init.ones_(self.b_rho)
            if self.fit_means:
