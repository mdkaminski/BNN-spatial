...
CWD = os.getcwd()
OUT_DIR = os.path.join(CWD, "output_1d_naive")
FIG_DIR = os.path.join(CWD, "figures_1d_naive")
...
hidden_dims = [50, 50, 50]  # list of hidden layer dimensions
...
# Create data set
n_train = 20  # number of observations
inds1 = np.random.randint(9*Xtest.size//16, 13*Xtest.size//16, size=n_train)
inds2 = np.random.randint(3*Xtest.size//16, 4*Xtest.size//16, size=0)
